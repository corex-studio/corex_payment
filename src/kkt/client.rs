...
impl Kkt {
    ...
    pub async fn connect(
        ...
        self.send("connect", "POST", None)
            .await
            .map(|v| ProcessSuccess::new("CONNECT_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new("CONNECT_FAIL", "Ошибка подключения к ККТ", e.to_string())
            })
    }
    ...
    pub async fn disconnect(
        ...
        self.send("disconnect", "POST", None)
            .await
            .map(|v| ProcessSuccess::new("DISCONNECT_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new("DISCONNECT_FAIL", "Ошибка отключения от ККТ", e.to_string())
            })
    }
    ...
    pub async fn check_connection(
        ...
        let response = self.send("check", "GET", None).await.map_err(|e| {
            ProcessError::new(
    ...
    pub async fn open_shift(
        ...
        self.send("open_shift", "POST", Some(&data))
            .await
            .map(|v| ProcessSuccess::new("OPEN_SHIFT_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new("OPEN_SHIFT_FAIL", "Ошибка открытия смены", e.to_string())
            })
    }
    ...
    pub async fn close_shift(
        ...
        let data = serde_json::json!({ "operator": operator });
        self.send("close_shift", "POST", Some(&data))
            .await
            .map(|v| ProcessSuccess::new("CLOSE_SHIFT_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new("CLOSE_SHIFT_FAIL", "Ошибка закрытия смены", e.to_string())
            })
    }
    ...
    pub async fn shift_status(
        ...
        self.send("shift_status", "GET", None)
            .await
            .map(|v| ProcessSuccess::new("SHIFT_STATUS_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new(
                    "SHIFT_STATUS_FAIL",
                    "Ошибка получения статуса смены",
                    e.to_string(),
                )
            })
    }
    ...
    pub async fn payment(
        ...
        self.send("payment", "POST", Some(&data))
            .await
    ...
    pub async fn refund(
        ...
        self.send("refund", "POST", Some(&data))
            .await
            .map(|v| ProcessSuccess::new("REFUND_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new(
                    "REFUND_FAIL",
                    "Ошибка при выполнении возврата",
                    e.to_string(),
                )
            })
    }
    ...
    pub async fn document(
        ...
        self.send("document", "GET", Some(&data))
            .await
            .map(|v| ProcessSuccess::new("DOCUMENT_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new("DOCUMENT_FAIL", "Ошибка получения документа", e.to_string())
            })
    }
    ...
    pub async fn info(
        ...
        self.send("info", "GET", None)
            .await
            .map(|v| ProcessSuccess::new("INFO_SUCCESSFUL", v))
            .map_err(|e| {
                ProcessError::new(
                    "INFO_FAIL",
                    "Ошибка получения информации о ККТ",
                    e.to_string(),
                )
            })
    }
